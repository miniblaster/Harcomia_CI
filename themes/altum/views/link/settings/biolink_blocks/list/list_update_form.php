...
<form name="update_biolink_" method="post" role="form">
    ...
    <input type="hidden" name="block_type" value="list" />
    <input type="hidden" name="biolink_block_id" value="<?= $row->biolink_block_id ?>" />

    <div class="notification-container"></div>

    <div class="form-group">
        <label for="<?= 'list_text_' . $row->biolink_block_id ?>"><i class="fa fa-fw fa-paragraph fa-sm text-muted mr-1"></i> <?= l('create_biolink_list_modal.text') ?></label>
        <textarea id="<?= 'list_text_' . $row->biolink_block_id ?>" class="form-control" name="text" maxlength="2048"><?= $row->settings->text ?></textarea>
        <small class="form-text text-muted"><?= l('create_biolink_list_modal.text_help') ?></small>
    </div>

    <div class="form-group">
        <label for="<?= 'list_icon_' . $row->biolink_block_id ?>"><i class="fa fa-fw fa-globe fa-sm text-muted mr-1"></i> <?= l('create_biolink_link_modal.input.icon') ?></label>
        <input id="<?= 'list_icon_' . $row->biolink_block_id ?>" type="text" name="icon" class="form-control" value="<?= $row->settings->icon ?>" placeholder="<?= l('create_biolink_link_modal.input.icon_placeholder') ?>" />
    ...
    <div class="collapse" id="<?= 'button_settings_container_' . $row->biolink_block_id ?>">
        <div <?= $this->user->plan_settings->custom_colored_links ? null : 'data-toggle="tooltip" title="' . l('global.info_message.plan_feature_no_access') . '"' ?>>
            <div class="<?= $this->user->plan_settings->custom_colored_links ? null : 'container-disabled' ?>">
                <div class="form-group">
                    <label for="<?= 'list_text_color_' . $row->biolink_block_id ?>"><i class="fa fa-fw fa-paint-brush fa-sm text-muted mr-1"></i> <?= l('create_biolink_link_modal.input.text_color') ?></label>
                    <input id="<?= 'list_text_color_' . $row->biolink_block_id ?>" type="hidden" name="text_color" class="form-control" value="<?= $row->settings->text_color ?>" required="required" />
                    <div class="text_color_pickr"></div>
                </div>

                <div class="form-group">
                    <label for="<?= 'list_text_alignment_' . $row->biolink_block_id ?>"><i class="fa fa-fw fa-align-center fa-sm text-muted mr-1"></i> <?= l('create_biolink_link_modal.input.text_alignment') ?></label>
                    <select id="<?= 'list_text_alignment_' . $row->biolink_block_id ?>" name="text_alignment" class="form-control">
                ...
                <div class="form-group">
                    <label for="<?= 'list_background_color_' . $row->biolink_block_id ?>"><i class="fa fa-fw fa-fill fa-sm text-muted mr-1"></i> <?= l('create_biolink_link_modal.input.background_color') ?></label>
                    <input id="<?= 'list_background_color_' . $row->biolink_block_id ?>" type="hidden" name="background_color" class="form-control" value="<?= $row->settings->background_color ?>" required="required" />
                    <div class="background_color_pickr"></div>
                </div>

                <div class="form-group">
                    <label for="<?= 'list_border_width_' . $row->biolink_block_id ?>"><i class="fa fa-fw fa-border-style fa-sm text-muted mr-1"></i> <?= l('create_biolink_link_modal.input.border_width') ?></label>
                    <input id="<?= 'list_border_width_' . $row->biolink_block_id ?>" type="range" min="0" max="5" class="form-control" name="border_width" value="<?= $row->settings->border_width ?>" required="required" />
                </div>

                <div class="form-group">
                    <label for="<?= 'list_border_color_' . $row->biolink_block_id ?>"><i class="fa fa-fw fa-fill fa-sm text-muted mr-1"></i> <?= l('create_biolink_link_modal.input.border_color') ?></label>
                    <input id="<?= 'list_border_color_' . $row->biolink_block_id ?>" type="hidden" name="border_color" class="form-control" value="<?= $row->settings->border_color ?>" required="required" />
                    <div class="border_color_pickr"></div>
                </div>

                <div class="form-group">
                    <label for="<?= 'list_border_radius_' . $row->biolink_block_id ?>"><i class="fa fa-fw fa-border-all fa-sm text-muted mr-1"></i> <?= l('create_biolink_link_modal.input.border_radius') ?></label>
                    <select id="<?= 'list_border_radius_' . $row->biolink_block_id ?>" name="border_radius" class="form-control">
                ...
                <div class="form-group">
                    <label for="<?= 'list_border_style_' . $row->biolink_block_id ?>"><i class="fa fa-fw fa-border-none fa-sm text-muted mr-1"></i> <?= l('create_biolink_link_modal.input.border_style') ?></label>
                    <select id="<?= 'list_border_style_' . $row->biolink_block_id ?>" name="border_style" class="form-control">
        ...
        <div class="form-group">
            <label for="<?= 'margin_items_y' . $row->biolink_block_id ?>"><?= l('create_biolink_list_modal.margin_items_y') ?></label>
            <input id="<?= 'margin_items_y' . $row->biolink_block_id ?>" type="range" name="margin_items_y" min="0" max="5" step="1" value="<?= $row->settings->margin_items_y ?>" class="form-control-range" />
        </div>

        <div class="form-group">
            <label for="<?= 'margin_items_x' . $row->biolink_block_id ?>"><?= l('create_biolink_list_modal.margin_items_x') ?></label>
            <input id="<?= 'margin_items_x' . $row->biolink_block_id ?>" type="range" name="margin_items_x" min="0" max="3" step="1" value="<?= $row->settings->margin_items_x ?>" class="form-control-range" />
        </div>
    </div>
